...
# temperature over time in Massachusetts.
#
...
Months_Since_1999 <- 1:nrow(MA_Temp_Monthly)

# Combine months vector and TAVG vector for the regression data set. 
TAVG_Regression <- cbind(Months_Since_1999, MA_Temp_Monthly$TAVG)
...
# Or a different one.
TAVG_Regression_Model <- lm(data = TAVG_Regression, TAVG ~ Months_Since_1999 + sin(2*pi*Months_Since_1999/12) + cos(2*pi*Months_Since_1999/12))
