...
(ggplot(data = VT_Weather_Monthly, aes(x = NewDate, y = TAVG, group = 1, color = "blue")) 
...
acf(VT_Weather_Monthly$TAVG, lag.max = 50, type = "correlation", main = expression('ACF'),
    ylab = expression('ACF, r'[k]), xlab = expression('Lag, k'))
acf(VT_Weather_Monthly$TAVG, lag.max = 50, type = "partial", main = expression('Partial ACF'),
    ylab = expression('ACF, r'[k]), xlab = expression('Lag, k'))
...
boxCox(VT_Weather_Monthly$TAVG ~ VT_Weather_Monthly$NewDate, lambda = seq(-3, 3, by = 1)) # Choose no lambda


# Hypothesis Tests to Assess Stationarity ---------------------------------

# Conduct KPSS tests for level and trend stationarity.
kpss.test(VT_Weather_Monthly$TAVG, null = "Level") # Stationary
kpss.test(VT_Weather_Monthly$TAVG, null = "Trend") # Stationary

# Perform an ADF test for unit-root stationarity.
adf.test(VT_Weather_Monthly$TAVG, alternative = "stationary") # Stationary

# Perform Canova-Hanson test for seasonal unit root stationarity.
ch.test(ts(VT_Weather_Monthly$TAVG, frequency = 12), sid = 12)


# Deasonalized Data -------------------------------------------------------

# Try differencing the data to eliminate seasonality. 
VT_TAVG_Seasonal <- VT_Weather_Monthly
VT_TAVG_Seasonal$TAVG <- c(array(NA, dim = c(12, 1)), diff(VT_TAVG_Seasonal$TAVG, 12))
VT_TAVG_Seasonal <- VT_TAVG_Seasonal[-c(1:12), ]

# Create time series plot for deseasonalized data.
(ggplot(data = VT_TAVG_Seasonal, aes(x = NewDate, y = TAVG, group = 1, color = "blue")) 
  + geom_line(color = "blue") 
  + xlab("Date") + ylab("Temperature (degrees Farenheit)") + ggtitle("Average Daily Temperature in Vermont by Month, Deseasonalized")
  + theme_bw() + geom_point(col = "red", pch = 1))

# Create ACF and PACF for time series. 
acf(VT_TAVG_Seasonal$TAVG, lag.max = 50, type = "correlation", main = expression('ACF'),
    ylab = expression('ACF, r'[k]), xlab = expression('Lag, k'))
acf(VT_TAVG_Seasonal$TAVG, lag.max = 50, type = "partial", main = expression('Partial ACF'),
    ylab = expression('ACF, r'[k]), xlab = expression('Lag, k'))
...
# Conduct KPSS tests for level and trend stationarity.
kpss.test(VT_TAVG_Seasonal$TAVG, null = "Level") # Stationary
kpss.test(VT_TAVG_Seasonal$TAVG, null = "Trend") # Stationary

# Perform an ADF test for unit-root stationarity.
adf.test(VT_TAVG_Seasonal$TAVG, alternative = "stationary") # Stationary

# Perform Canova-Hanson test for seasonal unit root stationarity.
ch.test(ts(VT_TAVG_Seasonal$TAVG, frequency = 12), sid = 12)


# Model Creation and Diagnostics ------------------------------------------

# Create ARIMA(0,0,0)x(0,1,0) model.  -------------------------------------
VT_TAVG_fit <- arima(VT_Weather_Monthly$TAVG, order = c(0, 0, 0), 
                     seasonal = list(order = c(0, 1, 0), period = 12)) 
summary(VT_TAVG_fit)

# Fiding BIC.
AIC(VT_TAVG_fit, k = log(length(VT_Weather_Monthly$TAVG)))
...
coeftest(VT_TAVG_fit, df = 227)

# Create ARIMA(1,0,0)x(0,1,1) model.  -------------------------------------
VT_TAVG_fit <- arima(VT_Weather_Monthly$TAVG, order = c(1, 0, 0), 
                     seasonal = list(order = c(0, 1, 1), period = 12)) 
summary(VT_TAVG_fit)
...
# Check the p-values of the model coefficients to see if they are significant. 
coeftest(VT_TAVG_fit, df = 226)

# Create ARIMA(1,0,1)x(0,1,1) model.  -------------------------------------
VT_TAVG_fit <- arima(VT_Weather_Monthly$TAVG, order = c(1, 0, 1), 
                     seasonal = list(order = c(0, 1, 1), period = 12)) 
summary(VT_TAVG_fit)
...
# Check the p-values of the model coefficients to see if they are significant. 
coeftest(VT_TAVG_fit, df = 225)

# Create ARIMA(3,0,0)x(0,1,1) model.  -------------------------------------
VT_TAVG_fit <- arima(VT_Weather_Monthly$TAVG, order = c(3, 0, 0), 
                     seasonal = list(order = c(0, 1, 1), period = 12)) 
summary(VT_TAVG_fit)
...
# Check the p-values of the model coefficients to see if they are significant. 
coeftest(VT_TAVG_fit, df = 224)


# Model Diganostics -------------------------------------------------------

# Confirm Stationarity with Unit Root Graph.
autoplot(VT_TAVG_fit) # Stationary

# Diagnostics Plots.
residuals_VT_TAVG_fit <- as.vector(residuals(VT_TAVG_fit))
predicted_VT_TAVG_fit <- as.vector(fitted(VT_TAVG_fit))
...
Res_ACF_VT_TAVG <- acf(residuals_VT_TAVG_fit, lag.max = 25, type = "correlation", main = expression('ACF of Residuals'), 
                       ylab = expression('ACF, r'[k]), xlab = expression('Lag, k'))
Res_PACF_VT_TAVG <- acf(residuals_VT_TAVG_fit, lag.max = 25, type = "partial", main = expression('Partial ACF of Residuals'), 
                        ylab = expression('ACF, r'[k]), xlab = expression('Lag, k'))
...
LJ_Stat <- c()
for(number in 1:25){
  variable1 <- Box.test(resid(VT_TAVG_fit), type = "Ljung", lag = number, fitdf=1)
...
VT_TAVG_Forecasts <- forecast(VT_TAVG_fit, h = 12, level = 95)
VT_TAVG_Forecasts

# Plot the forecasts. 
(autoplot(ts(data = VT_Weather_Monthly$TAVG), series = "Data") + autolayer(fitted(VT_TAVG_fit), series = "ARIMA(1,0,0) x (0,1,1)")
  + autolayer(VT_TAVG_Forecasts, series = "Forecast")
  ...
  + ggtitle("Average Daily Temperature in Vermont Forecasts by Month") + theme(legend.position = "bottom"))


# Yet ANOTHER way to graph the data, but forecasts ONLY.
dyTAVG <- xts(as.data.frame(cbind(gen_array(VT_TAVG_Forecasts), VT_Weather_Monthly_Evaluation$TAVG)),
              order.by = seq(as.Date("2019-01-01"), as.Date("2019-12-01"), by = "months"))
colnames(dyTAVG) <- c("Lo 95", "Hi 95","Point Forecast", "TAVG")

dygraph(data = dyTAVG, main = 'Average Daily Temperature in Vermont Forecasts by Month ',
...
TAVG_Model1 <- arima(VT_Weather_Monthly$TAVG, order = c(0, 0, 0), 
                     seasonal = list(order = c(0, 1, 0), period = 12)) 
TAVG_Model2 <- arima(VT_Weather_Monthly$TAVG, order = c(0, 0, 0), 
                     seasonal = list(order = c(0, 1, 1), period = 12)) 
TAVG_Model3 <- arima(VT_Weather_Monthly$TAVG, order = c(1, 0, 0), 
                     seasonal = list(order = c(0, 1, 1), period = 12)) 
TAVG_Model4 <- arima(VT_Weather_Monthly$TAVG, order = c(1, 0, 1), 
                     seasonal = list(order = c(0, 1, 1), period = 12)) 
TAVG_Model5 <- arima(VT_Weather_Monthly$TAVG, order = c(3, 0, 0), 
                     seasonal = list(order = c(0, 1, 1), period = 12)) 


# Plot data and various models.
(autoplot(ts(data = VT_Weather_Monthly$TAVG), series = "Data", size = 1) 
  ...
  + scale_colour_manual(values = c('Data'='black', 'ARIMA(0,0,0) x (0,1,0)'='#E30022',
                        ...
                        breaks = c('Data', 'ARIMA(0,0,0) x (0,1,0)', 'ARIMA(0,0,0) x (0,1,1)', 
                                   ...
                                   'ARIMA(3,0,0) x (0,1,1)'))
  + guides(color=guide_legend(title="")) + xlab('Date ') + ylab('Temperature (degrees Farenheit)') + theme_bw()
  + ggtitle("Average Daily Temperature in Vermont by Month") + theme(legend.position = "bottom"))


# Create a table that includes all error measures for comparing the models. 
TAVG_Comparison_Measures <- rbind(cbind(summary(TAVG_Model1), TAVG_Model1$aic, AIC(TAVG_Model1, k = log(length(VT_Weather_Monthly$TAVG)))),
                                  cbind(summary(TAVG_Model2), TAVG_Model2$aic, AIC(TAVG_Model2, k = log(length(VT_Weather_Monthly$TAVG)))),
                                  cbind(summary(TAVG_Model3), TAVG_Model3$aic, AIC(TAVG_Model3, k = log(length(VT_Weather_Monthly$TAVG)))),
                                  cbind(summary(TAVG_Model4), TAVG_Model4$aic, AIC(TAVG_Model4, k = log(length(VT_Weather_Monthly$TAVG)))), 
                                  cbind(summary(TAVG_Model5), TAVG_Model5$aic, AIC(TAVG_Model5, k = log(length(VT_Weather_Monthly$TAVG)))))
